...
```bash
# Download specific version (replace v1.0.56 with desired version)
wget https://github.com/linescripts/timenzo/raw/v1.0.56/Timenzo-1.0.56-ae39cf2.sh

# Make executable
chmod +x Timenzo-1.0.56-ae39cf2.sh

# Run installer
./Timenzo-1.0.56-ae39cf2.sh
```

Available files in each version branch:
- `timenzo-installer.sh` - Generic filename (same across all versions)
- `Timenzo-1.0.56-ae39cf2.sh` - Version-specific filename
