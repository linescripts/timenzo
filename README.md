...
```bash
# Download specific version (replace v1.0.55 with desired version)
wget https://github.com/linescripts/timenzo/raw/v1.0.55/Timenzo-1.0.55-5cfb874.sh

# Make executable
chmod +x Timenzo-1.0.55-5cfb874.sh

# Run installer
./Timenzo-1.0.55-5cfb874.sh
```

Available files in each version branch:
- `timenzo-installer.sh` - Generic filename (same across all versions)
- `Timenzo-1.0.55-5cfb874.sh` - Version-specific filename
